# -*- coding: utf-8 -*-
"""
Created on Tue Apr 24 00:28:22 2018
...
for z in range(10):
    ...
    A_mat = A_df.pivot(index ='User-ID', columns ='ISBN', values ='Ratings').fillna(0).values
    
    
    P,Q,train_errors,test_errors = svd_sgd(A_mat,test_mat,L,lmbda,eta,epoch)
    
    ERROR_TRAIN.append(train_errors[-1])
    
    ERROR_TEST.append(test_errors[-1])
...
plt.figure()
plt.plot(per_data, ERROR_TRAIN,'r',label='Training Error')
plt.plot(per_data, ERROR_TEST,'b',label='Testing Error')
plt.xlabel('x10% of Data')
plt.ylabel('RMSE')
